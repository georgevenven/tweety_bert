...
def calculate_distribution_similarity(heatmap1, heatmap2):
   ...
   if np.isnan(similarity):
       print("Warning: NaN similarity detected")
       print(f"heatmap1 sum: {np.sum(heatmap1)}")
       print(f"heatmap2 sum: {np.sum(heatmap2)}")
       return 0.0
...
def plot_similarity_comparison(similarity_matrix, stats_results, save_dir, bird_id):
   ...
   ax2.set_ylabel('Similarity (Bhattacharyya Coefficient)')
...
def plot_pairwise_comparisons(embedding_outputs, dataset_indices, save_dir, bird_id):
   ...
   individual_heatmaps = plot_separate_and_combined_umaps(
       embedding_outputs,
       dataset_indices,
       save_dir,
       bird_id,
...
def analyze_multiple_birds(npz_files, save_dir):
    ...
    # Compile results
    if results:
        # Add the new bird comparison matrix BEFORE creating the DataFrame
        plot_bird_comparison_matrix(results, save_dir)
        ...
        # Add the similarity summary plots
...
def plot_separate_and_combined_umaps(embedding_outputs, dataset_indices, save_dir, bird_id, 
    ...
    # Average before and after periods
    ...
    ax.set_xlabel('UMAP Dimension 1')
    ax.set_ylabel('UMAP Dimension 2')
    plt.colorbar(ax.imshow(rgb_image, extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]], 
                          origin='lower'), ax=ax)
    fig.tight_layout()
...
def plot_all_heatmaps_combined(embedding_outputs, dataset_indices, save_dir, bird_id, 
    ...
    # Create and plot combined before (bottom middle)
    ...
    before_heatmap, _, _ = np.histogram2d(before_data[:, 0], before_data[:, 1], 
                                        bins=[xedges, yedges])
    before_heatmap = before_heatmap / before_heatmap.max()
    im = ax_before.imshow(before_heatmap.T, extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]], 
                         origin='lower', cmap='Purples', vmax=0.1)
    ax_before.set_title("Combined Before", fontsize=14)
    ax_before.set_xlabel('UMAP 1')
    ax_before.set_ylabel('UMAP 2')
    plt.colorbar(im, ax=ax_before)
    ...
    after_heatmap, _, _ = np.histogram2d(after_data[:, 0], after_data[:, 1], 
                                       bins=[xedges, yedges])
    after_heatmap = after_heatmap / after_heatmap.max()
    im = ax_after.imshow(after_heatmap.T, extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]], 
                        origin='lower', cmap='Greens', vmax=0.1)
    ax_after.set_title("Combined After", fontsize=14)
    ax_after.set_xlabel('UMAP 1')
    ax_after.set_ylabel('UMAP 2')
    plt.colorbar(im, ax=ax_after)
...
def plot_similarity_summary(results_df, save_dir):
    ...
    ax1.set_ylabel('Similarity (Bhattacharyya Coefficient)')
...
def plot_bird_comparison_matrix(results_list, save_dir):
    """
    Create a bird-to-bird comparison matrix visualization.
    
    Parameters:
    - results_list: List of dictionaries containing results for each bird
    - save_dir: Directory to save the plot
    """
    n_birds = len(results_list)
    similarity_matrix = np.zeros((n_birds, n_birds))
    
    # Calculate similarities between birds using their combined distributions
    for i in range(n_birds):
        for j in range(n_birds):
            # Calculate average similarity between all conditions of birds i and j
            bird_i_matrix = results_list[i]['similarity_matrix']
            bird_j_matrix = results_list[j]['similarity_matrix']
            
            # Average all pairwise comparisons between birds
            similarity = np.mean(bird_i_matrix)  # within-bird for diagonal
            if i != j:
                similarity = np.mean([bird_i_matrix.mean(), bird_j_matrix.mean()]) / 2
            
            similarity_matrix[i, j] = similarity
    
    # Create the heatmap
    plt.figure(figsize=(10, 8))
    mask = np.triu(np.ones_like(similarity_matrix), k=1)  # Mask upper triangle
    ...
    sns.heatmap(similarity_matrix, 
                ...
                fmt='.3f',
                cmap='RdYlBu_r',
                xticklabels=[f'Bird {i+1}' for i in range(n_birds)],
                yticklabels=[f'Bird {i+1}' for i in range(n_birds)],
                cbar_kws={'label': 'Average Similarity'})
    
    plt.title('Bird-to-Bird Similarity Matrix', pad=20)
    
    # Calculate and add some statistics
    mean_within = np.mean(np.diag(similarity_matrix))
    mean_between = np.mean(similarity_matrix[~np.eye(similarity_matrix.shape[0], dtype=bool)])
    
    plt.figtext(0.02, 0.02, 
                f'Mean within-bird similarity: {mean_within:.3f}\n'
                f'Mean between-bird similarity: {mean_between:.3f}',
                fontsize=10)
    
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'bird_comparison_matrix.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(save_dir, 'bird_comparison_matrix.svg'), bbox_inches='tight')
    plt.close()
