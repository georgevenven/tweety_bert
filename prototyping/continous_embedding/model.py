...
class TweetyBERT(nn.Module):
    def __init__(self, d_transformer, nhead_transformer, num_freq_bins, num_labels, dropout=0.1, transformer_layers=3, dim_feedforward=128, m = 33, p = 0.01, alpha = 1, length = 500, pos_enc_type="relative"):
        ...
        self.decoder1 = nn.Linear(self.embedding_size, 128)
        self.decoder2 = nn.Linear(128, 196)
    ...
    def train_forward(self, spec):
        ...
        embedded_x = self.embedding(x.permute(0,2,1))
        intermediate_outputs["embedding"] = embedded_x.clone()

        decoded1 = self.decoder1(embedded_x)
        decoded2 = self.decoder2(decoded1)

        intermediate_outputs["decoder1"] = decoded1.clone()
        intermediate_outputs["decoder2"] = decoded2.clone()
    ...
    def mse_loss(self, spec, predictions, mask, embedded_x, decoded2):
        epsilon = 1e-6
        alpha = self.alpha
        ...
        cosine_loss = 1 - cosine_similarity(predictions, embedded_x)
        # Apply mask
        ...
        mse_loss = mse_loss_func(decoded2, reshaped_spec)

        if masked_indices.sum() > 0:
            masked_loss = cosine_loss[masked_indices].mean()
        else:
            masked_loss = torch.tensor(epsilon).to(predictions.device)

        if unmasked_indices.sum() > 0:
            unmasked_loss = cosine_loss[unmasked_indices].mean()
        else:
        ...
        combined_loss = combined_loss + mse_loss.mean()

        # mse loss is before mean 
        return combined_loss, masked_loss, unmasked_loss, cosine_loss