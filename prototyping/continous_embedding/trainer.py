...
class ModelTrainer:
    ...
    def validate_model(self, step, spec):
        ...
        with torch.no_grad():
            ...
            val_loss, masked_seq_acc, unmasked_seq_acc, *rest = self.model.mse_loss(predictions=output, spec=spec , mask=mask, embedded_x=intermediate_outputs["embedding"], decoded2=intermediate_outputs["decoder2"])
    ...
    def train(self, continue_training=False, training_stats=None, last_step=0):
        ...
        while step < self.max_steps:
            ...
            with autocast():
                output, mask, spec, intermediate_outputs = self.model.train_forward(spec)
                loss, *rest = self.model.mse_loss(predictions=output, spec=spec, mask=mask, embedded_x=intermediate_outputs["embedding"], decoded2=intermediate_outputs["decoder2"])
