...
class TweetyBERT(nn.Module):
    ...
    def mse_loss(self, predictions, mask, spec, intermediate_layers, smoothness_weight=.5, shift_distance=1):
        middle_attention_layer = intermediate_layers["transformerProjection"]
        batches, time_bins, features = middle_attention_layer.shape
        intermediate_activations = middle_attention_layer.reshape(batches, time_bins, features)
        # permute the last two dimensions of the intermediate activations for consistency sake
        intermediate_activations = intermediate_activations.permute(0, 2, 1)

        # shifted intermediate activations
        shifted_intermediate_activations = torch.zeros_like(intermediate_activations)
        shifted_intermediate_activations[:, :, shift_distance:] = intermediate_activations[:, :, :-shift_distance]

        # cosine similarity between shifted and original
        cosine_similarity_matrix = torch.nn.functional.cosine_similarity(shifted_intermediate_activations, intermediate_activations, dim=2)
        cosine_similarity_matrix[:, :shift_distance] = 0  # omit cosine similarity for the first 5 time bins
        mean_cosine_similarity = torch.mean(cosine_similarity_matrix)
        ...
        combined_loss += ((1 - mean_cosine_similarity + epsilon) * smoothness_weight)
