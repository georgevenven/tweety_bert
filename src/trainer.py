...
class ModelTrainer:
    ...
    def validate_model(self, step, spec, vocalization):
        ...
        with torch.no_grad():
            ...
            val_loss, masked_seq_acc, unmasked_seq_acc, *rest = self.model.mse_loss(predictions=output, spec=spec , mask=mask, intermediate_layers=intermediate_outputs, vocalization=vocalization)
    ...
    def train(self, continue_training=False, training_stats=None, last_step=0):
        ...
        while step < self.max_steps:
            ...
            with autocast():
                output, mask, _, intermediate_outputs = self.model.train_forward(spec)
                loss, *rest = self.model.mse_loss(predictions=output, spec=spec, mask=mask, intermediate_layers=intermediate_outputs, vocalization=vocalization)
            ...
            with autocast():
                val_loss, avg_masked_seq_acc, avg_unmasked_seq_acc = self.validate_model(step, spec=validation_spec, vocalization=validation_vocalization)
